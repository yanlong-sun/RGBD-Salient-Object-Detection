...
clear all;
tic;
imgRoot='C:\Users\ELONSUN\Desktop\script\Dataset\LFSD\Image\';% test image path
supdir='C:\Users\ELONSUN\Desktop\script\3_deepSaliency\Result\LFSD\SuperPixels\';% the superpixel label file path
rawSalDir='C:\Users\ELONSUN\Desktop\script\3_deepSaliency\Result\Processed-LFSD\DeepMAP\';
saldir='C:\Users\ELONSUN\Desktop\script\SaliencyMap\LFSD\Ours\';% the output path of the saliency map
nfeature=3;
pa=5;
if ~isdir(supdir)
mkdir(supdir);
...
%%    
  ...
  %%  这里改了
   rawSalMapname = [rawSalDir imnames(ii).name(1:end-4)  '.png']; 
   map=double(imread(rawSalMapname));
...
imwrite(sal,[saldir imnames(ii).name(1:end-4) '.png']); 
end
t=toc;
